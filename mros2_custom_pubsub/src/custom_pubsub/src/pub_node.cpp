...
private:
  void timer_callback()
  {
    ...
    message.weight = float_t 63.5;
    RCLCPP_INFO(this->get_logger(), "Publishing msg: { name: '%s', height: '%s'cm, weight: '%s'kg }", message.name.c_str(), message.height.c_str(), message.weight.c_str());
